...
We will see a quick setup to configure the ``proxmox`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``proxmox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``proxmox`` formulas under ``/srv/salt/proxmox/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/proxmox/parameters/dns:domain/
    ln -s dns:domain /srv/salt/proxmox/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/proxmox/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/proxmox/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``proxmox`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``proxmox`` formulas under ``/srv/salt/proxmox/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/proxmox/parameters/roles
...
We create a configuration for the role ``proxmox/server`` in ``/srv/salt/proxmox/parameters/roles/proxmox/server.yaml``:
...
We create another configuration for the role ``proxmox/client`` in ``/srv/salt/proxmox/parameters/roles/proxmox/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles proxmox/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles proxmox/client
...
.. code-block:: console
    ...
    mv /srv/salt/proxmox/parameters/dns:domain/  '/srv/salt/proxmox/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-proxmox-as-json:
      test.nop:
        - name: {{ proxmox | json }}
...
Here is an example based on `template-formula/proxmox/config/file.sls`_:
...
This ``sls`` file expose a ``proxmox`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ proxmox['winner'] }}
...
.. _template-formula/proxmox/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/proxmox/config/file.sls
